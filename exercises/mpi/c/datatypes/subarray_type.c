...
int main(int argc, char **argv)
{
    ...
    MPI_Type_create_subarray(2,sizes,subsizes,offsets,MPI_ORDER_C,MPI_INT,&subtype);
    ...
    if (rank == 0){
      MPI_Send(array,1,subtype,1,11,MPI_COMM_WORLD);
    }
    else if (rank == 1){
      MPI_Recv(array,1,subtype,0,11,MPI_COMM_WORLD,&status);
    }
